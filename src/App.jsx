...
export default function App() {
      ...
      {hasVelData
        ...
        {sorted.map((ci, rank) => {
          ...
          return (
            ...
            {/* Expanded detail — same as CraftingTab but only for craftable items */}
  ...
  const CraftingTab = useMemo(() => {
    ...
    .filter(([, discs]) => discs.includes(disc))
    .map(([name]) => name)
    : [];

    // Helper: given an itemId and needed count, find characters that have it
    // but the crafting character(s) don't have enough
    const findItemOnOtherChar = (itemId, needed) => {
      if (!data.charInventoryByChar) return null;
      const charInv = data.charInventoryByChar;

      // How much do the crafters collectively have in their bags?
      const crafterHas = crafterNames.reduce((sum, n) => sum + (charInv[n]?.[itemId] || 0), 0);
      if (crafterHas >= needed) return null; // crafters have enough, no note needed

      // Find other chars that have any of it
      const others = Object.entries(charInv)
        ...
        {sorted.slice(craftPage*PAGE_SIZE, (craftPage+1)*PAGE_SIZE).map(ci => {
          ...
          return (
            ...
            {isOpen && (
              ...
              {(() => {
                // Recursively render the recipe tree, showing intermediary nodes
                const matDetailMap = Object.fromEntries(ci.matDetails.map(m => [m.itemId, m]));
                const renderNode = (node, depth = 0) => {
                  const rows = [];
                  for (const [idx, child] of node.children.entries()) {
                    const item = data.itemMap[child.itemId];
              ...
              const owned = cacheRef.current.ownedMap?.[child.itemId] || 0;

              if (isIntermediary) {
                // Check if we have enough owned already (no need to show sub-ingredients)
                const canMakeIt = checkFulfillment(child.itemId, child.count, cacheRef.current.resolvedRecipes, cacheRef.current.ownedMap);
                // "fulfilled from owned" means we literally have enough in bags — no crafting needed
                ...
                // Leaf: base material
                // Use matDetails if available; otherwise derive from owned vs needed.
                // Note: child.count is the raw tree count (for full recipe, ignoring intermediate ownership).
                // matDetails uses flatLeaves counts which account for owned intermediaries — use that if available.
                ...
                return ci.tree ? renderNode(ci.tree) : ci.matDetails.map(m => {
              ...
              {/* Buy order suggestions: ingredients where placing a buy order saves gold ──
                ...
                {(() => {
                  ...
                  const suggestions = mustBuyMats.map(m => {
                    const priceData = data.priceMap[m.itemId];
                    ...
                    // Only suggest if buy orders actually get filled (buyFillsPerHr tells us sellers hit bids)
                  ...
                  return (
                    ...
                    {suggestions.map(s => {
                      const item = data.itemMap[s.itemId];
                      return (
                ...
                {ci.matSellPaths.length > 0 && ci.matSellNet > ci.profitNet && (
                  ...
                  {ci.matSellPaths.map((p, i) => (
                    ...
                    {(p.path || []).map((it, j) => (
                      ...
                      <span className={`rar-${data.itemMap[it.itemId]?.rarity}`}>{it.name} ×{it.count}</span>
                ...
                {!ci.canCraft && ci.cheapAcquire.path.filter(p => p.needed > 0).length > 0 && (
                  ...
                  {ci.cheapAcquire.path.filter(p => p.needed > 0 && (p.name || p.unitPrice > 0)).map((p, i) => {
                    const displayName = data.itemMap[p.itemId]?.name || VENDOR_PRICES[p.itemId]?.name || p.name || `Item ${p.itemId}`;
                    return (
